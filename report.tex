...
The Bash script provides a menu-driven terminal interface with the following capabilities:
\begin{itemize}
    \item \textbf{Select user}: Lists all unique usernames and validates the selection.
    \item \textbf{View recent games}: Displays the last 20 games for the selected user, colour-coded by death cause.
    \item \textbf{Analytics}: Computes average score, average survival time, and death cause breakdown, optionally filtered by a timestamp cutoff.
    \item \textbf{Delete entries}: Remove records by username, by exact timestamp, or clean malformed lines.
    \item \textbf{Log rotation}: Creates a timestamped \texttt{.tar.gz} backup of \texttt{history.txt}, trims the live file to its last 10 entries, and auto-deletes backups older than the five most recent.
    \item \textbf{Sort and view}: Sorts entries by timestamp, username, or score.
\end{itemize}
